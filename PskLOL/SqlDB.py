    ...
    def _create_db(self):
        try:
            ...
            if not exists:
                ...
                cur.execute(sql.SQL("CREATE DATABASE {} WITH ENCODING 'utf8' TEMPLATE template0").format(sql.Identifier(self.db_name)))
...
if __name__ == "__main__":
    ...
    db = SqlDB(user=config.user, host=config.host, password=config.password, db_name=config.db_name)
    print(db.conn)
    db.close()
